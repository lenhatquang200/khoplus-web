$(document).ready(async function () {
    ...
    let response = await tools.ajaxGet("/manufacturing-groups")
    if (response.success) {
        const list = []
        let num = 1
        ...
        $('#modalFormSubmit').submit(async e => {
            ...
            if (type) {
                const response = await tools.ajaxPost("/manufacturing-groups", {
                    name: name
                })
                if (response.success) {
                ...
                const response = await tools.ajaxPut("/manufacturing-groups/" + id, {
                    name: name
                })
                if (response.success) {
        ...
        async function deleteRow(data) {
            const response = await tools.ajaxDelete("/manufacturing-groups/" + data.id)
            if (response.success) {
