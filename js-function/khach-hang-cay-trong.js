$(document).ready(async function () {
    ...
    let response = await tools.ajaxGet("/plants")
    if (response.success) {
        const list = []
        let num = 1
        ...
        $('#modalFormSubmit').submit(async e => {
            ...
            if (type) {
                const response = await tools.ajaxPost("/plants", {
                    name: name
                })
                if (response.success) {
                    ...
                    table.addRow(row)
                    tools.toast("success", "Cây trồng", "Tạo mới thành công.")
                } else {
                    tools.toast("error", "Cây trồng", "Lỗi, vui lòng kiểm tra lại.")
                }
            } else {
                //update row
                ...
                const response = await tools.ajaxPut("/plants/" + id, {
                    name: name
                })
                if (response.success) {
                    table.updateRow({id: id, trId: trId, num: num}, {name: name})
                    tools.toast("success", "Cây trồng", "Cập nhật thành công.")
                } else {
                    tools.toast("error", "Cây trồng", "Lỗi, vui lòng kiểm tra lại.")
                }
            }
        ...
        async function deleteRow(data) {
            const response = await tools.ajaxDelete("/plants/" + data.id)
            if (response.success) {
                table.deleteRow(data)
                tools.toast("success", "Cây trồng", "Xóa thành công.")
            } else {
                tools.toast("error", "Cây trồng", "Lỗi, vui lòng kiểm tra lại.")
            }
        }
